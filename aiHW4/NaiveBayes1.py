...
class NaiveBayes(Predictor):
    ...
    def train(self, instances):
        ...
        features = instances[0].getFeatureVector().getFeatureVec().keys() #instances[0].feature_vector.feature_vec.keys()
        # Training model
    ...
    def _mean(self, instances, feature):  # Can be improved with np
        s = 0
        for instance in instances:
            s += instance.getFeatureVector().getFeatureVec()[feature]#instance.feature_vector.feature_vec[feature]
    ...
    def _variance(self, instances, feature, mean):  # Can be improved with np
        ...
        for instance in instances:
            ...
            # instance.feature_vector.feature_vec[feature]
    ...
    def predict(self, instance):
        posteriors = {}
        features = instance.getFeatureVector().getFeatureVec().keys()#instance.feature_vector.feature_vec.keys()
        for label in self.class_priors:
            posteriors[label] = self.class_priors[label]
            for feature in features:
                posteriors[label] *= self._gaussian(instance.getFeatureVector().getFeatureVec()[feature], self.likelihoods[label][feature])
